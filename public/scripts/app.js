...
$(document).ready(function() {
  ...
  $(document).ready(function() {
    ...
    $("#editSongsModalBody").on(
      "click",
      "button.btn-danger",
      handleDeleteSongClick
    );
    $("#editSongsModal").on(
      "click",
      "button#editSongsModalSubmit",
      handleUpdateSongsSave
    );
  });
  ...
  function handleUpdateSongsSave(e) {
    ...
    $modal.find("form").each(function() {
      ...
      song.name = $(this)
        .find("input.song-name")
        .val();
      song.trackNumber = $(this)
        .find("input.song-trackNumber")
        .val();
  ...
  function updateSongs(albumId, songs) {
    ...
    songs.forEach(function(song) {
      var putRequest = $.ajax({
        ...
        data: song,
        error: function(err) {
          console.log("Error updating song", song.name, err);
        }
      });
  ...
  function handleAlbumSaveClick() {
    var albumId = $(this)
      .parents(".album")
      .data("album-id"); // $(this).closest would have worked fine too
    ...
    var songEditFormHtmlStrings = songs.map(function(song) {
      ...
      <form class="form-inline" id="${song._id}" data-album-id="${albumId}" >
        <div class="form-group">
          <input type="text" class="form-control song-trackNumber" value="${
            song.trackNumber
          }">
        </div>
        <div class="form-group">
          <input type="text" class="form-control song-name" value="${
            song.name
          }">
        </div>
        <div class="form-group">
